...
import { FileText, Search, Plus, Edit3, Trash2, Eye, Download, ArrowLeft, CheckCircle, XCircle } from 'lucide-react';

export default function DocumentTemplatesPage() {
  const router = useRouter();
  const [search, setSearch] = useState('');
  const [templates] = useState([
    { id: '1', name: 'Patient Consent Form', category: 'clinical', status: 'active', format: 'PDF' },
    { id: '2', name: 'Discharge Summary', category: 'clinical', status: 'active', format: 'DOCX' },
    { id: '3', name: 'Referral Letter', category: 'clinical', status: 'active', format: 'DOCX' },
    { id: '4', name: 'Lab Report Template', category: 'laboratory', status: 'active', format: 'PDF' },
    { id: '5', name: 'Prescription Pad', category: 'pharmacy', status: 'inactive', format: 'PDF' },
    { id: '6', name: 'Invoice Template', category: 'finance', status: 'active', format: 'PDF' },
  ]);
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div className="flex items-center gap-3"><button onClick={() => router.push('/dashboard/admin/templates')} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><ArrowLeft className="w-4 h-4" /></button>
        <div><h1 className="text-2xl font-extrabold text-slate-800 dark:text-white">Document Templates</h1><p className="text-sm text-slate-400 mt-1">{templates.length} template(s)</p></div>
      </div>
      <div className="relative"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
        <input type="text" placeholder="Search document templates..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm outline-none" /></div>
